...
describe('Component', () => {
  beforeEach(() => {
    ...
    test('test multistoryboard 2', async ()  => {
      ...
      expect(data.tags).toBe(true)
      expect(data.layerslist).toBe(false)
      expect(Object.keys(data.shortcuts).sort()).toEqual(['tags', 'close', 'home', 'zoomin', 'info', 'fullscreen', 'hide', 'overlay', 'zoomout', 'shortcut', 'next', 'autorun', 'prev', 'textoverlay'].sort())
      wrapper.destroy()
    })
    test('test multistoryboard 3', async ()  => {
      ...
      expect(data.layerslist).toBe(false)
      expect(Object.keys(data.shortcuts).sort()).toEqual(['close', 'home', 'zoomin', 'info', 'fullscreen', 'hide', 'overlay', 'zoomout', 'shortcut', 'next', 'autorun', 'prev', 'textoverlay'].sort())
      wrapper.destroy()
    })
    test('test multistoryboard 4', async ()  => {
      ...
      expect(data.tags).toBe(false)
      expect(data.layerslist).toBe(false)
      expect(Object.keys(data.shortcuts).sort()).toEqual(['close', 'home', 'zoomin', 'info', 'fullscreen', 'hide', 'overlay', 'zoomout', 'shortcut', 'next', 'autorun', 'prev', 'textoverlay', 'transcription'].sort())
      wrapper.destroy()
    })
})
