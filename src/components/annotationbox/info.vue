...
<div id="information" style="height: auto;" class="info content">
    ...
    <button class="infolink buttonlink" v-on:click="send.sendMessage({'function':'switchShown', 'args': 'additionalinfoshown'});" v-if="parent.settings.additionalinfo">{{parent.settings.additionalinfotitle}}</button>
    <div v-if="parent.booleanitems.additionalinfoshown" v-html="parent.settings.additionalinfo" class="imageinfo"></div>
    <button class="infolink buttonlink" v-on:click="send.sendMessage({'function':'switchShown', 'args': 'tocshown'});" v-if="parent.basecompontent.range && parent.basecompontent.toc.length > 1">{{parent.basecompontent.toctitle}}</button>
    ...
    <button class="infolink buttonlink annoinfolink" v-on:click="send.sendMessage({'function':'switchShown', 'args': 'annoinfoshown'});" v-if="parent.annoinfo.text">Annotation information</button>
    <div v-if="parent.booleanitems.annoinfoshown" class="annoinfo">
        ...
        <div v-for="annoinfo in parent.annoinfo.annodata" v-bind:key="annoinfo.position" v-bind:id="'data_' + annoinfo.position">
            <div class="title"><button class="buttonlink" v-on:click="send.sendMessage({'function': 'next', 'args': annoinfo.position});">{{annoinfo.title}}</button></div>
            <div class="additionaltext" v-html="annoinfo.additionaltext"></div>
        </div>
        </div>
    </div>

    <button class="infolink buttonlink imageinfolink" v-if="parent.imageinfo.text" v-on:click="send.sendMessage({'function':'switchShown', 'args': 'imageinfoshown'});">{{parent.imageinfo.label}}</button>
