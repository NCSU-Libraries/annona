...
<div v-bind:id="$parent.seadragonid + '_annotation'" class="annotation" v-bind:class="[$parent.booleanitems.isexcerpt ? 'excerpt' : 'fullcontent', $parent.textposition, $parent.settings.toolbarposition ? $parent.settings.toolbarposition + '_menu_annotation' : '', $parent.settings.hide_toolbar ? 'no_toolbar_annotation' : '']" v-show="$parent.shown" tabindex="0">
      <span v-if="!$parent.settings.hide_annocontrols && $parent.settings.hide_annocontrols !== true" id="annotation_controls">
        <button class="annocontrols_button" id="close_button" v-if="$parent.shortcuts['close']" v-on:click="$parent.shown = false; $parent.setDefaultButtons();" aria-label="close content box button">
          <i class="fas fa-times"></i>
        </button>
        <button v-if="$parent.shortcuts['hide']" id="hide_button" class="annocontrols_button"  v-on:click="$parent.sendMessage({'function': 'hide', 'args': '','toolbar': true});" aria-label="collapse content box button">
          <span v-html="$parent.buttons.hide_button"></span>
        </button>
        <button id="playpause_button" class="annocontrols_button" v-on:click="$parent.sendMessage({'function': 'playpause', 'args': '','toolbar': true});" v-if="$parent.settings.tts && $parent.shortcuts['playpause']" aria-label="play/pause text to speech button">
          <span v-html="$parent.buttons.playpause"></span>
        </button>
        <button id="tags_button" v-if="$parent.shortcuts['tags']" class="annocontrols_button" v-on:click="$parent.sendMessage({'function': 'clickButton', 'args': 'tags','toolbar': true});" aria-label="toggle tags button">
          <span v-html="$parent.buttons.tags"></span>
        </button>
        <button id="info_button" v-if="($parent.imageinfo || $parent.annoinfo.text) && $parent.shortcuts['info']" class="annocontrols_button" v-on:click="$parent.sendMessage({'function': 'clickButton', 'args': 'info','toolbar': true});" aria-label="toggle info button">
          <span v-html="$parent.buttons.info"></span>
        </button>
        <button id="transcription_button" class="annocontrols_button" v-if="$parent.hastranscription && $parent.shortcuts['transcription']" v-on:click="$parent.sendMessage({'function': 'setShownData', 'args': $parent.booleanitems.istranscription ? 'anno' : 'transcription','toolbar': true});" aria-label="toggle transcription button">
          <span v-html="$parent.buttons.anno"></span>
        </button>
        <button v-if="$parent.shortcuts['prev']" id="previousButton" v-on:click="$parent.sendMessage({'function': 'next', 'args': 'prev','toolbar': true});" v-bind:class="{ 'inactive' : $parent.prev_inactive }" class="annocontrols_button" aria-label="previous annotation button">
          <i class="fa fa-arrow-left"></i>
          <span class="toolbartext">Previous Annotation</span>
        </button>
        <button v-if="$parent.shortcuts['next']" id="nextButton" v-on:click="$parent.sendMessage({'function': 'next', 'args': 'next','toolbar': true});" v-bind:class="{ 'inactive' : $parent.next_inactive }" class="annocontrols_button" aria-label="next annotation button">
        ...
        <div v-for="(groupval, groupkey) in $parent.groupTagDict" v-bind:key="groupkey" class="tagslist">
          <div class="taggroup tags" v-if="groupkey" v-bind:id="groupkey + '_tags'">
            ...
            <span v-bind:style="'background: ' + groupval.color" class="tagscount">
              <span v-if="!$parent.settings.hide_tagcount">{{groupval.count}}</span>
            </span>
            </div>
          </div>
          <div v-bind:class="{ 'subtags' : groupkey }" class="tags" v-for="tagdict in groupval.tags" v-bind:id="tagdict.key + '_tags'" v-bind:key="tagdict.key" >
            ...
            <span v-bind:style="'background: ' + tagdict.color" class="tagscount">
              <span v-if="!$parent.settings.hide_tagcount">{{tagdict.count}}</span>
            </span>
            </div>
          </div>
        </div>
      ...
      <div id="information" style="height: auto;" v-if="$parent.shown == 'info'" class="info content">
        <div class="imagetitle"><h1>{{$parent.imagetitle}}</h1></div>
