...
describe('Component', () => {
  beforeEach(() => {
    ...
    test('test manifesturl range', async ()  => {
      ...
      expect(Object.keys(sbd.shortcuts).sort()).toEqual(['autorun', 'reload', 'close', 'textoverlay', 'fullscreen', 'hide', 'home', 'info', 'next', 'nextanno', 'overlay', 'prev', 'prevanno', 'keyboard', 'zoomin', 'zoomout'].sort());
      wrapper.destroy()
    })
    ...
    test('test manifesturl with per page', async ()  => {
      ...
      expect(sbd.imagetitle).toBe("Technician Basketball Special, November 1991: Text of this page (word level)");
      expect(sbd.layerslist).toEqual([{"checked": true, "label": "Layer 1", "opacity": 1, "tile": "https://iiif.lib.ncsu.edu/iiif/technician-basketballspecial-1991-11_0001/info.json", "xywh": ""}]);
      expect(Object.keys(sbd.shortcuts).sort()).toEqual(['close', 'reload', 'textoverlay', 'fullscreen', 'hide', 'home', 'info', 'next', 'nextanno', 'overlay', 'prev', 'prevanno', 'keyboard', 'zoomin', 'zoomout'].sort());
